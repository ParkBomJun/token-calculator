...
- **CSP 화이트리스트** — 페이지의 모든 네트워크 통신을 자체 호스트 + `api.anthropic.com`으로
  제한하는 Content-Security-Policy를 선언했다. 만약 의존성 오염(공급망 공격)으로 악성 코드가
  끼어들어도 키를 내보낼 수 있는 목적지가 없다. 새 벤더 API를 붙일 때마다 이 목록에
  명시적으로 추가해야 하며, 그 자체가 통신 대상의 감사 기록이 된다.
