...
- **호출은 OpenRouter 단일 경유** — 키 하나로 전 카탈로그를 커버하고, 네이티브 usage
  패스스루가 캘리브레이션에서 ×1.000으로 검증돼 있어 토큰·비용 집계가 정확하다.
  카탈로그의 OpenRouter 슬러그는 `models.json`의 `openrouter` 필드
  (gemini-3-pro는 OpenRouter에 텍스트 모델이 없어 제외 — 2026-07-12 확인).
- **벤더 CORS 실측 (2026-07-12)** — 프리플라이트(OPTIONS + Origin) 실측 결과
  Anthropic·OpenAI·DeepSeek·GLM(bigmodel.cn)·OpenRouter 모두 허용, Gemini도 전용 헤더
  (`x-goog-api-key`)로는 허용. 즉 **전 벤더 직행 모드가 기술적으로 가능**하지만,
  CSP 유출 표면 최소화 원칙(§6)에 따라 OpenRouter만 열었다. 직행 모드가 필요해지면
  이 실측 기록이 근거가 된다.
...
- **CSP 화이트리스트** — 페이지의 모든 네트워크 통신을 자체 호스트 + `api.anthropic.com`
  + `openrouter.ai`(v0.4)로 제한하는 Content-Security-Policy를 선언했다. 만약 의존성
  오염(공급망 공격)으로 악성 코드가 끼어들어도 키를 내보낼 수 있는 목적지가 없다.
  새 벤더 API를 붙일 때마다 이 목록에 명시적으로 추가해야 하며, 그 자체가 통신 대상의
  감사 기록이 된다. 도메인 하나가 늘 때마다 유출 표면도 늘어나므로, v0.4에서 6개 벤더
  전부 CORS가 열려 있음을 실측(§1.7)하고도 OpenRouter 하나만 추가했다.
