...
async function measureVendor(v, key, locals) {
  ...
  for (const spec of v.models) {
    ...
    // 이전 실행에서 이미 6샘플 완료된 모델은 재측정하지 않는다 (재측정하려면 결과 파일에서 해당 모델 삭제)
    if (existingResult?.vendors?.[v.name]?.[model]?._summary?.samples === Object.keys(SAMPLES).length) {
      console.log(`  [${model}] 이미 측정 완료 — 스킵`)
      continue
    }
    const entry = {}
...
try { existingResult = JSON.parse(await readFile(OUT, 'utf8')) } catch {}
const done = []
